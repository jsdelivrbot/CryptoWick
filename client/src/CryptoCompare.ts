...
export function loadAggregate1HourCandlesticks(fromSymbol: string, toSymbol: string, exchangeName: string, hoursPerCandlestick: number): Promise<TradeAnalysis> {
  return fetch(`https://min-api.cryptocompare.com/data/histohour?fsym=${fromSymbol}&tsym=${toSymbol}&aggregate=${hoursPerCandlestick}&e=${exchangeName}`)
    .then(response => {
      if (!response.ok) {
        console.log("Error fetching data from CryptoWatch.");
        return;
      ...
      return new TradeAnalysis(fromSymbol + toSymbol, exchangeName, `${hoursPerCandlestick}h`, openTimes, opens, highs, lows, closes, volumes);
    });
}